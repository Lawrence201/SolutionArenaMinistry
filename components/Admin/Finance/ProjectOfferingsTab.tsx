...
interface ProjectOfferingsTabProps {
    refreshStats?: () => void;
}

export default function ProjectOfferingsTab({ refreshStats }: ProjectOfferingsTabProps) {
    ...
    const handleViewReceipt = (record: Record) => {
    ...
    const handleDelete = async (id: string) => {
        if (!confirm('Are you sure you want to delete this project offering record?')) return;
        try {
            const res = await fetch(`/api/finance/project-offerings?id=${id}`, { method: 'DELETE' });
            const json = await res.json();
            if (json.success) {
                fetchProjectOfferings();
    ...
    const handleSaveEdit = async (updatedData: any) => {
        ...
        try {
            const payload = {
                ...updatedData,
                // Ensure collection method maps correctly from payment_method if that's what EditModal provides
                collection_method: updatedData.collection_method || updatedData.payment_method || 'Cash'
            };
            const res = await fetch(`/api/finance/project-offerings?id=${selectedRecord.transaction_id}`, {
                method: 'PUT',
            ...
            console.error('Error saving project offering:', error);
            throw error;
        }
    };
                                            ...
                                            <tr key={record.transaction_id} className="eorp_exprow">
                                                ...
                                                <td>
                                                    <div className="gorp_actbtns">
                                                        <button className="borp_acticon" title="View Details" onClick={() => handleViewReceipt(record)}>
                                                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
